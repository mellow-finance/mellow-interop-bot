...
def validate_confirmation_accounting(
    ...
    if len(missing_confirmations_set) != len(missing_confirmations):
        raise Exception(f"Duplicate addresses found in missing_confirmations: {missing_confirmations}")
    ...
    if overlap:
        raise Exception(f"Addresses found in both confirmations and missing_confirmations: {list(overlap)}")
